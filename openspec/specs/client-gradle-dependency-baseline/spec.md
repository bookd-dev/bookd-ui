...
- **THEN** Gradle wrapper SHALL use Gradle 9.5.0
...
- **AND** Android debug build SHALL pass with `./gradlew :androidApp:assembleDebug`.

### Requirement: Client build warnings are minimized
The client project SHALL use non-deprecated source APIs where practical.
...
#### Scenario: AGP 9 KMP application split is complete
- **WHEN** AGP 9 is used with the Kotlin Multiplatform shared module
- **THEN** the shared KMP module SHALL use `com.android.kotlin.multiplatform.library`
- **AND** the Android application SHALL use a dedicated `com.android.application` subproject.
- **AND** temporary AGP 9 compatibility properties SHALL NOT be required.
